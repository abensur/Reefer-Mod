#!/usr/bin/env python3
"""
Generate a 512x512 mod icon for the Bad Habits mod
...
def create_mod_icon():
    """Create a 512x512 mod icon featuring a joint"""
    ...
    # Center and rotate the joint slightly for dynamic look
    center_x = 256
    center_y = 256

    # Draw joint body (horizontal, centered, larger)
    joint_length = 320
    joint_width = 48
    start_x = center_x - joint_length // 2
    start_y = center_y - joint_width // 2

    # Twisted tip (left side)
    twist_start = start_x
    for i in range(3):
        y_offset = start_y + joint_width // 2 - 16 + i * 16
        draw.ellipse(
    ...
    draw.rectangle(
        [(filter_start, start_y), (filter_start + filter_width, start_y + joint_width)],
        fill=filter_color
    )
    draw.rectangle(
        [(filter_start, start_y + 4), (filter_start + filter_width, start_y + joint_width - 4)],
    ...
    for i in range(3):
        ...
        circle_y = start_y + joint_width // 2
        draw.ellipse(
    ...
    # Add subtle shadow below joint
    shadow_y = start_y + joint_width + 8
    for i in range(3):
        alpha = int(40 * (3 - i) / 3)
        draw.ellipse(
            [(start_x + 40 - i * 8, shadow_y + i * 4),
             (start_x + joint_length - 80 + i * 8, shadow_y + 8 + i * 4)],
            fill=(0, 0, 0, alpha)
        )
...
if __name__ == "__main__":
    ...
    icon.save("src/main/resources/badhabits_logo.png")
    print("✓ Generated badhabits_logo.png (512x512)")
